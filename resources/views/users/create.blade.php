...
<div class="mb-6 flex items-center justify-between">
    <div>
        ...
        <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white">Add Team Member</h1>
        <p class="text-gray-500 dark:text-gray-400 mt-1">Create a new clinic account</p>
    </div>
</div>
    ...
    <form action="{{ route('users.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="email" name="email" required value="{{ old('email') }}"
                   class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary outline-none transition dark:text-white">
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">They'll use this to log in</p>
        </div>

        <div>
            <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Password *</label>
            <input type="password" name="password" required minlength="8" value="{{ old('password') }}"
                   class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary outline-none transition dark:text-white">
        ...
        <div>
            ...
            <select name="role" required class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary outline-none transition dark:text-white">
                ...
                <option value="clinic_admin" {{ old('role') === 'clinic_admin' ? 'selected' : '' }}>Clinic Admin (Full access)</option>
                <option value="doctor" {{ old('role') === 'doctor' ? 'selected' : '' }}>Doctor (Clinical access)</option>
                <option value="receptionist" {{ old('role') === 'receptionist' ? 'selected' : '' }}>Receptionist (Front desk)</option>
                <option value="accountant" {{ old('role') === 'accountant' ? 'selected' : '' }}>Accountant (Financial access)</option>
            </select>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Roles determine what features they can access</p>
        </div>
